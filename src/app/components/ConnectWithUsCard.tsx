...
interface ConnectWithUsCardProps {
  /** Short title (e.g. "Brand not found", "Product not available") */
  title: string;
...
export default function ConnectWithUsCard({
  title,
  backLink,
  whatsappMessage = "Hi, I'm looking for something that wasn't on your site. Can you help me get it at a great deal?",
}: ConnectWithUsCardProps) {
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-8 text-center shadow-lg dark:border-white/10 sm:p-12">
      ...
      {backLink && (
        <Link href={backLink.href} className="mt-8 inline-block text-sm font-medium text-primary hover:underline">
          ← {backLink.text}
        </Link>
      )}
    </div>

    ...
    <>
      ...
      <div className="-mx-4 w-[calc(100%+2rem)] space-y-6 px-4 sm:-mx-6 sm:w-[calc(100%+3rem)] sm:px-6">
        <div className="rounded-xl border border-border bg-card p-6 dark:border-white/10">
          <h2 className="mb-4 text-lg font-semibold text-foreground">{t('contactSection')('getInTouch')}</h2>
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              <strong className="text-foreground">{t('contactSection')('email')}:</strong>{' '}
            ...
            {contact?.phone && (
              <p className="text-sm text-muted-foreground">
                <strong className="text-foreground">{t('contactSection')('phone')}:</strong>{' '}
            ...
            {contact?.address && (
              <p className="text-sm text-muted-foreground">
                <strong className="text-foreground">{t('contactSection')('address')}:</strong> {contact.address}
              </p>
            )}
            {instagramUrl && (
              <p className="text-sm text-muted-foreground">
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            {(contact?.whatsapp || contact?.phone) && (
              <a
                href={getWhatsAppUrl(contact?.whatsapp || contact?.phone || '', t('contactSection')('whatsappInquiryMessage'))}
                target="_blank"
                rel="noopener noreferrer"
            ...
            {contact?.mapDirectionsUrl && (
              <a
                ...
                aria-label={t('aria')('getDirectionsMaps')}
              >
                <svg className="h-5 w-5" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
                  <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z" />
                </svg>
                {t('contactSection')('getDirections')}
              </a>
            )}
          </div>
        </div>
              ...
              <div className="flex flex-wrap gap-2">
                {contact?.mapPlaceUrl && (
                  <a
                    ...
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 rounded-lg border border-border px-4 py-2 text-sm font-medium no-underline transition-colors hover:bg-muted dark:border-white/10"
                    aria-label={t('aria')('viewOnMaps')}
                  >
                    <svg className="h-5 w-5" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
                      <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z" />
                    </svg>
                    {t('contactSection')('viewOnMaps')}
                  </a>
                )}
                {contact?.mapDirectionsUrl && (
                  <a
                    ...
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground no-underline transition-opacity hover:opacity-90"
                    aria-label={t('aria')('getDirections')}
                  >
                    <svg className="h-5 w-5" viewBox="0 0 24 24" fill="currentColor" aria-hidden>

  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!cancelled) setError(err instanceof Error ? err.message : String(err ?? 'Failed to load'));
      } finally {
    ...
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (Array.isArray(output) && Array.isArray(output[0])) {
          embedding = output[0];
        } else if (output && typeof output === 'object' && output !== null && 'data' in output) {
          const data = (output as { data?: Iterable<number> }).data;
          embedding = data ? Array.from(data) : [];
        } else if (Array.isArray(output)) {
          embedding = output;
        } else if (output != null && typeof (output as Iterable<number>)[Symbol.iterator] === 'function') {

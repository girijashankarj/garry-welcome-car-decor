...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!product) return {};
  const tProducts = getTranslations(locale as Locale, 'productsDetail');
  const productName = (() => {
    const v = tProducts(`${slug}-name`);
    return v !== `${slug}-name` ? v : product.name;
  })();
  const productDesc = (() => {
    const v = tProducts(`${slug}-description`);
    return v !== `${slug}-description` ? v : product.description;
  })();
  return {
...
export default async function ProductDetailPage({ params }: Props) {
  ...
  const tErrors = getTranslations(locale as Locale, 'errors');
  const tContact = getTranslations(locale as Locale, 'contactSection');
  const tProducts = getTranslations(locale as Locale, 'productsDetail');
    ...
    return (
      <div className="mx-auto max-w-2xl">
        <nav className="mb-6 flex flex-wrap items-center gap-1 text-sm text-muted-foreground">
          ...
          <span>/</span>
          <Link href={`/${locale}/products`} className="hover:text-primary hover:underline">
            {tNav('products')}
          </Link>
          <span>/</span>
          <span className="text-foreground">{tErrors('productNotAvailable')}</span>
        </nav>
        <ConnectWithUsCard
          title={tErrors('productNotAvailable')}
          backLink={{ href: '/products', text: tCommon('backToProducts') }}
          whatsappMessage={tCommon('whatsappProductMessage')}
        />
      </div>
    );
  }
  ...
  const productDesc = (() => {
    const v = tProducts(`${product.slug}-description`);
    return v !== `${product.slug}-description` ? v : product.description;
  })();
  ...
  return (
    <>
      <ProductJsonLd product={{ ...product, name: productName, description: productDesc }} />
      ...
      <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-lg dark:border-white/10">
        <div className="grid gap-8 p-6 sm:grid-cols-2 sm:p-8">
          ...
          <div>
            ...
            <p className="mb-6 text-muted-foreground">{productDesc}</p>
            {product.price != null && (
            ...
            <div className="flex flex-wrap gap-3">
              {whatsappNumber ? (
                <a
                  href={getWhatsAppUrl(whatsappNumber, productName)}
                  target="_blank"
      ...
      <div className="mt-8 text-center">
        <Link
          href={category ? `/${locale}/categories/${category.slug}` : `/${locale}/categories`}
          className="text-sm font-medium text-primary hover:underline"
        >
          {t('backTo').replace('{name}', categoryName ?? tNav('categories'))}
        </Link>
      </div>
    </div>
    </>
